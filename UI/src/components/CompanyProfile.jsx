  ...
  return (
    <div className="bg-surface-container-high rounded-xl overflow-hidden border border-outline-variant transition-all hover:border-secondary">
      ...
      <div className="p-4">
        ...
        <h3 className="text-headline-sm text-on-surface mb-2 line-clamp-2">{event.name}</h3>
        <div className="flex items-center gap-2 text-on-surface-variant mb-4">
          <span className="material-symbols-outlined" style={{ fontSize: '18px' }}>location_on</span>
          <span className="text-body-md truncate">{event.location}</span>
        </div>
        {event.artistName && (
          <p className="text-label-md text-on-surface-variant mb-3">{event.artistName}</p>
        )}
        <div className="flex items-center justify-between mt-4">
          <div className="flex flex-col">
            <span className="text-label-sm text-on-surface-variant">{isFree ? 'Admission' : 'From'}</span>
...
export default function CompanyProfile() {
  ...
  useEffect(() => {
    ...
    Promise.allSettled([
      fetch(`http://localhost:8080/api/discovery/companies`, { headers, signal: controller.signal })
        .then((res) => { if (!res.ok) throw new Error(`Company fetch: ${res.status}`); return res.json(); })
        .then((companies) => companies.find((c) => c.companyName === companyName) ?? null),

      fetch(`http://localhost:8080/api/discovery/companies/${encodeURIComponent(companyName)}/events`, {
        headers,
        signal: controller.signal,
      })
        .then((res) => { if (!res.ok) throw new Error(`Events fetch: ${res.status}`); return res.json(); }),
    ])
  ...
  return (
    <div className="bg-background text-on-surface min-h-screen pb-24">
      ...
      <header className="relative w-full h-[397px] pt-16">
        <div className={`absolute inset-0 bg-gradient-to-br ${headerGradient} z-0`} />
            ...
            <>
              ...
              {company ? (
                <p className="text-body-md text-on-surface-variant max-w-sm">
                  Rating: {company.rating > 0 ? `${company.rating.toFixed(1)} / 5.0` : 'No ratings yet'}
                </p>
              ) : (
                <p className="text-body-md text-on-surface-variant max-w-sm">
        ...
        {error ? (
          ...
          <div className="flex flex-col gap-6">
            {loading ? (
              ...
              <div className="flex flex-col items-center justify-center py-20 text-center">
                <span className="material-symbols-outlined text-on-surface-variant mb-4" style={{ fontSize: '48px' }}>event_busy</span>
                <p className="text-headline-sm text-on-surface mb-2">No events yet</p>
                <p className="text-body-md text-on-surface-variant">Check back soon for upcoming events.</p>
              </div>
            ) : (
              events.map((event, i) => <EventCard key={event.eventId ?? i} event={event} />)
            )}
          </div>
        )}
      </main>
