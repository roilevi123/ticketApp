...
public class WaitingQueueTests {
    ...
    public WaitingQueueTests(UserService userService, CompanyService companyDomain, EventService eventDomain, QueueService queueDomain, initTheSystem initTheSystem) {
    ...
    public void setUp(){
        ...
        companyDomain.CreateCompany("creator",token1);
        eventDomain.createEvent(token1, "Rock Festival", "Arctic Monkeys", EventType.CONFERENCE, 100.0, new Date(), "Tel Aviv", "creator", getMapArea());
    ...
    public boolean TestSequentialQueueEntry1() {
        ...
        String status1 = queueDomain.checkStatus(eventId, "user1");

        String status2 = queueDomain.checkStatus(eventId, "user2");
    ...
    public boolean TestQueuePositionIncrements2() {
        ...
        for (int i = 1; i <= 100; i++) {
            queueDomain.checkStatus(eventId, "dummyUser" + i);
        }

        String status101 = queueDomain.checkStatus(eventId, "waiter1");
        String status102 = queueDomain.checkStatus(eventId, "waiter2");
    ...
    public boolean TestConcurrentQueueAccess3() {
        ...
        companyDomain.CreateCompany("creator3", token);
        eventDomain.createEvent(token, "Concurrent Fest", "Artist", EventType.CONFERENCE, 100.0, new Date(), "Haifa", "creator3", getMapArea());
        ...
        for (int i = 0; i < threadCount; i++) {
            ...
            service.submit(() -> {
                try {
                    latch.await();
                    results.add(queueDomain.checkStatus(eventId, uname));
                } catch (Exception e) {
                    e.printStackTrace();
                }
            });
    ...
    public boolean TestConcurrentQueueOverflow4() {
        ...
        companyDomain.CreateCompany("creator4", token);
        eventDomain.createEvent(token, "Overfill Fest", "Artist", EventType.CONFERENCE, 100.0, new Date(), "Eilat", "creator4", getMapArea());
    ...
    public boolean TestConcurrentSameUserRefresh5() {
        ...
        companyDomain.CreateCompany("creator5", token);
        eventDomain.createEvent(token, "Refresh Fest", "Artist", EventType.CONFERENCE, 100.0, new Date(), "Jerusalem", "creator5", getMapArea());
        ...
        for (int i = 1; i <= 50; i++) {
            queueDomain.checkStatus(eventId, "waitingUser" + i);
        }
        ...
        for (int i = 0; i < threadCount; i++) {
            service.submit(() -> {
                try {
                    latch.await();
                    results.add(queueDomain.checkStatus(eventId, "waitingUser25"));
                } catch (Exception e) {
                    e.printStackTrace();
                }
            });
