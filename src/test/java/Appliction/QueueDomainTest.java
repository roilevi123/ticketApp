...
class QueueDomainTest {
    ...
    void setUp() {
        ...
        queueDomain = new QueueDomain(queueRepository);
    }


    @Test
    void checkStatus_NewUser_Authorized() {
        String status = queueDomain.checkStatus(EVENT_ID, "User1");
    ...
    void checkStatus_HitCapacity_Waiting() {
        for (int i = 0; i < 100; i++) {
            queueDomain.checkStatus(EVENT_ID, "User" + i);
        }

        String status = queueDomain.checkStatus(EVENT_ID, "Waiter1");
        assertEquals("WAITING_POSITION_1", status);

        String status2 = queueDomain.checkStatus(EVENT_ID, "Waiter2");
    ...
    void checkStatus_ReleaseSpace_ProgressesQueue() {
        ...
        queueDomain.checkStatus(EVENT_ID, "NextInLine");
        ...
        String status = queueDomain.checkStatus(EVENT_ID, "NextInLine");
        assertEquals("AUTHORIZED", status);
    }

    @Test
    void checkStatus_UserAlreadyAuthorized_RemainsAuthorized() {
        queueDomain.checkStatus(EVENT_ID, "User1");
        String status = queueDomain.checkStatus(EVENT_ID, "User1");
        assertEquals("AUTHORIZED", status);
    }
    ...
    void checkStatus_OrderIntegrity_NoJumping() {
        for (int i = 0; i < 100; i++) {
            String res = queueDomain.checkStatus(EVENT_ID, "Active" + i);
            assertEquals("AUTHORIZED", res);
        }

        queueDomain.checkStatus(EVENT_ID, "FirstWaiter");
        String status2Before = queueDomain.checkStatus(EVENT_ID, "SecondWaiter");
        assertEquals("WAITING_POSITION_2", status2Before);

        queueRepository.removeFromQueue(EVENT_ID, "FirstWaiter");

        String status2After = queueDomain.checkStatus(EVENT_ID, "SecondWaiter");
